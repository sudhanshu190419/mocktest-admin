...
import { Hourglass, SignOut, ShieldCheck } from '@phosphor-icons/react';

export default function PendingApprovalPage() {
  const { teacherProfile, signOut, loading } = useAuth();
  const router = useRouter();

  // If the user's status changes (e.g. admin approves), redirect to teacher dashboard
  React.useEffect(() => {
    if (!loading && teacherProfile) {
      if (teacherProfile.accountStatus === 'approved') {
        router.push('/teacher');
      } else if (teacherProfile.accountStatus !== 'pending') {
        // Not pending anymore but not approved either — redirect to relevant page
        const statusToRoute: Record<string, string> = {
          rejected: '/account-rejected',
          suspended: '/account-suspended',
          inactive: '/account-inactive',
        };
        const route = statusToRoute[teacherProfile.accountStatus];
        if (route) router.push(route);
      }
    }
  }, [teacherProfile, loading, router]);
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-slate-900 via-navy-900 to-slate-900 flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-md mx-auto">
        {/* Card */}
        <div className="rounded-3xl bg-white shadow-2xl border border-slate-100 overflow-hidden">
          {/* Illustration area */}
          <div className="h-48 bg-gradient-to-br from-amber-50 via-amber-100/50 to-orange-50 flex items-center justify-center relative overflow-hidden">
            ...
            <div className="relative flex flex-col items-center gap-2">
              <div className="w-20 h-20 rounded-full bg-amber-100 border-4 border-amber-200 flex items-center justify-center">
                <Hourglass size={36} weight="duotone" className="text-amber-600" />
              </div>
            </div>
          </div>
          ...
          <div className="p-8 pt-6 text-center space-y-4">
            ...
            <p className="text-sm text-slate-500 leading-relaxed max-w-sm mx-auto">
              Your institute administrator is reviewing your account.
