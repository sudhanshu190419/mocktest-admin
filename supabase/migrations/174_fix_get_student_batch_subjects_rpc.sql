-- ============================================================================
-- Migration: 174 — Fix Get Student Batch Subjects RPC (Parity Correction)
--
...
-- CONTEXT & PURPOSE
-- ════════════════════════════════════════════════════════════════════════════
-- Corrects and enhances migration 173's get_student_batch_subjects() RPC to
-- guarantee 100% security and behavioral parity with legacy access semantics:
--
--   1. Permanent Course Owners: Includes students with full-course purchase
--      entitlements (course_enrollments.enrollment_type = 'purchase') who may
--      not have an active student_subscriptions row (Phase 11K.2 parity).
--   2. Course Visibility & Multi-Tenancy: Explicitly requires
--      c.status = 'published', c.institute_id = v_institute_id, and
--      c.deleted_at IS NULL (preventing SECURITY DEFINER bypass of course RLS).
--   3. Soft-Deleted Subjects: Excludes soft-deleted subjects via
--      s.deleted_at IS NULL.
--
-- Preserves existing signature, return structure, and client compatibility.
...
BEGIN
  ...
  student_entitled_courses AS (
    ...
    UNION
    -- Permanent course owners (Phase 11K.2 standalone purchase entitlement)
    SELECT ce.course_id
  ...
  batch_course_map AS (
    ...
    ORDER BY cb.batch_id, cb.created_at ASC NULLS LAST, cb.course_id
  )
  SELECT
    bs.batch_subject_id,
    bs.batch_id,
...
COMMENT ON FUNCTION public.get_student_batch_subjects(uuid[]) IS
  'Returns authorized batch subjects with subject, batch, and course metadata for the authenticated student. SECURITY DEFINER; enforces institute isolation, active batch assignment, subscription window and permanent ownership entitlement, course publication status, and active non-deleted subjects.';

REVOKE ALL ON FUNCTION public.get_student_batch_subjects(uuid[]) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.get_student_batch_subjects(uuid[]) FROM anon;
