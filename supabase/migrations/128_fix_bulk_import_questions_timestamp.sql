-- ============================================================================
-- Migration 128: Fix Question Bank Bulk Import Timestamp Constraint Failure
--
-- Root Cause:
--   Multiple separate calls to `clock_timestamp()` inside the INSERT VALUES
--   clause caused `approved_at` to evaluate microseconds earlier than
--   `created_at`, violating `ck_questions_approved_at check (approved_at >= created_at)`.
--
-- Fix:
--   Captures `v_now := clock_timestamp()` once per iteration and assigns it
--   uniformly to `created_at`, `updated_at`, and `approved_at`, guaranteeing
--   `approved_at >= created_at` is always TRUE.
-- ============================================================================
