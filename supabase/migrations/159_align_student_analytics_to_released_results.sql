-- ============================================================================
-- Migration: 159 — Align Student Analytics to Released Results Policy
--
...
--
-- Ensures that only completed attempts with an existing released mock_results
-- record (mock_results.is_released = true) contribute to student analytics.
--
-- Benefits:
--   - Prevents score and correctness leakage on scheduled / manual mock tests
--     whose results are withheld from students.
--   - Practice tests with immediate release (result_release_mode = 'immediate')
--     continue contributing to analytics immediately upon submission.
--   - Unreleased, unevaluated, or abandoned attempts cannot pollute analytics.
--   - Aligns Subject/Chapter analytics with the Dashboard Summary and Test Results.
--
-- @module migrations/159
-- ============================================================================

-- Drop any obsolete 0-argument overload to ensure zero PostgREST ambiguity
...
COMMENT ON FUNCTION public.get_student_subject_analytics(uuid) IS
  'Returns a JSON array of subject analytics for released test results of the authenticated student. Accepts an optional p_test_id parameter (default NULL). If NULL, aggregates all released tests. If provided, aggregates only attempts for that specific test.';
...
COMMENT ON FUNCTION public.get_student_chapter_analytics() IS
  'Returns a JSON array of chapter-wise analytics for released test results of the authenticated student.';
...
COMMENT ON FUNCTION public.get_student_weak_chapters() IS
  'Returns a JSON array of chapters ordered from weakest to strongest based on percentage score from released test results.';
  ...
  RETURN (
    ...
    SELECT coalesce(
      json_agg(
        ...
        ORDER BY (CASE WHEN ca.max_score > 0 THEN round((ca.total_score / ca.max_score) * 100, 2) DESC NULLS LAST
      ),
      '[]'::json
    )
    FROM chapter_agg ca
...
COMMENT ON FUNCTION public.get_student_strong_chapters() IS
  'Returns a JSON array of chapters ordered from strongest to weakest based on percentage score from released test results.';
