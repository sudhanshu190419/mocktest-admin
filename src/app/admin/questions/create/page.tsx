...
export default function AdminCreateQuestionPage() {
  ...
  const handleSave = useCallback(() => {
    if (!validate()) return;
    if (!user?.id) {
      setSaveError('Admin user profile not found. Please log in again.');
      return;
    }

    if (!instituteId) {
      setSaveError('Institute not found. Please contact support.');
      return;
    }
  ...
  }, [formData, user, instituteId, createQuestion, router, validate]);

  return (
    <div className="max-w-3xl space-y-6">
      <PageHeader
        title="Create Question"
        description="Add a new question directly to the published question bank"
        breadcrumbs={[
        ...
        actions={
          <div className="flex items-center gap-3">
            <button
              ...
              disabled={isPending}
              className="rounded-xl border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 transition-colors hover:bg-gray-50 disabled:opacity-40 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800"
            >
              Cancel
            </button>
            <button
              ...
              onClick={handleSave}
              disabled={isPending}
              className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-all hover:bg-blue-700 disabled:opacity-50"
            >
              {isPending ? (
                <>
      ...
      {/* Subject/Chapter/Topic creation feedback banner */}
      {addSubjectFeedback && (
        <div
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-700 dark:bg-gray-900">
        <QuestionForm
          ...
          onSubjectChange={(sId) =>
            setFormData((prev) => ({ ...prev, subjectId: sId, chapterId: '', topicId: '' }))
          }
          onChapterChange={(cId) =>
            setFormData((prev) => ({ ...prev, chapterId: cId, topicId: '' }))
          }
          data={formData}
