...
function ScheduledCard({ notification, onDelete, onPublishNow }: {
  notification: Notification;
  onDelete: (id: string) => void;
  onPublishNow: (notification: Notification) => void;
}) {
  // For scheduled notifications that haven't been delivered yet,
  // treat the createdAt + 1 day as a synthetic scheduled time
  ...
  return (
    <div className="rounded-xl border border-gray-200 bg-white p-5 transition-all hover:border-gray-300 hover:shadow-md dark:border-gray-700 dark:bg-gray-900 dark:hover:border-gray-600">
      ...
      <div className="mt-4 flex items-center gap-2 border-t border-gray-100 pt-3 dark:border-gray-700">
        <button
          type="button"
          onClick={() => onPublishNow(notification)}
          className="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700"
        >
          Publish Now
        </button>
        <Link
          href={`/teacher/notifications/create?duplicate=${notification.notificationId}`}
          className="rounded-lg border border-gray-200 px-3 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-400 dark:hover:bg-gray-800"
        >
          Edit
        </Link>
        <button
...
// ─── Placeholder scheduled notifications (backend will provide real ones) ──
...
function useScheduledNotifications(userId: string | undefined) {
  ...
  // Treat unread notifications as "scheduled" — in production a dedicated
  // scheduled_notifications table or `scheduled_at` field would be used.
  const scheduled = useMemo(() => {
    if (!unreadData?.notifications) return [];
    // Take recent unread notifications as synthetic scheduled items
...
export default function ScheduledNotificationsPage() {
  ...
  const handlePublishNow = (_notification: Notification) => {
    // In production, this would call a publishNow API
    // For now, navigate to the notification detail
    router.push(`/teacher/notifications/${_notification.id}`);
  };
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <EmptyState
          ...
          description="Scheduled notifications will appear here. You can schedule an announcement for later delivery."
          action={
            <Link
              href="/teacher/notifications/create"
