...
import { usePyqPurchaseList } from '@/hooks/admin/useCommerce';
...
import type { PyqPurchaseListItem } from '@/services/admin/commerceService';
...
export default function PyqPurchasesPage() {
  ...
  const filters = useMemo(() => ({
    ...
    accessType: accessTypeFilter || undefined,
    instituteId: instituteId ?? undefined,
  }), [activeFilter, accessTypeFilter, debouncedSearch, instituteId]);
  ...
  const {
    ...
    error,
    refetch,
  } = usePyqPurchaseList(filters, { page, pageSize });

  // ── Table Columns ────────────────────────────────────────────────────
  const columns: Column<PyqPurchaseListItem>[] = useMemo(() => [
    {
      key: 'studentName',
      header: 'Student',
      render: (item) => (
    ...
    },
    {
      ...
      render: (item) => (
        <div className="text-xs text-gray-500">
          <p>{formatDate(item.purchasedAt)}</p>
          <p className="text-[10px] text-gray-400">{formatTimeAgo(item.purchasedAt)}</p>
        </div>
      ),
    },
    {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="PYQ Purchases"
        description="Verify PYQ package purchases and student access."
        breadcrumbs={[
      ...
      {isError && (
        <div className="rounded-xl border border-red-200 bg-red-50 p-4 dark:border-red-800 dark:bg-red-900/20">
          <p className="text-sm font-medium text-red-800 dark:text-red-300">
            Failed to load PYQ purchases: {error instanceof Error ? error.message : 'Unknown error'}
          </p>
        </div>
      )}
      ...
      <div className="flex flex-wrap items-end gap-3">
        ...
        <Select
          ...
          onChange={(v) => handleFilterChange(setAccessTypeFilter, v)}
      ...
      <DataTable
        ...
        emptyState={
          <EmptyState
            icon={<Package size={40} weight="thin" />}
            title="No PYQ purchases found"
            description={
              debouncedSearch || activeFilter || accessTypeFilter
                ? 'Try adjusting your search or filters.'
                : 'PYQ purchases will appear here once students buy packages.'
            }
          />
        }
        page={page}
