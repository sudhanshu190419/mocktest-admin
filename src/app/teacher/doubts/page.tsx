...
import { useTeacherDoubts, useTeacherDoubtSubjects } from '@/hooks/doubt/useDoubt';
...
type Segment = 'all' | 'me' | 'unassigned';
...
export default function TeacherDoubtsPage() {
  ...
  // Any filter change resets to page 1 (avoids landing on an empty page
  ...
  // Current teacher id — resolved via the established identity service so
  // "Assigned to Me" compares against teacher_details.teacher_id (NOT the
  // profile id). Resolved once; cached in-memory by the service.
  const [teacherId, setTeacherId] = useState<string | null>(null);
  useEffect(() => {
    let active = true;
    resolveTeacherIdentity().then((identity) => {
      if (active) setTeacherId(identity?.teacherId ?? null);
    });
    return () => {
      active = false;
    };
  ...
  // Debounce the search input (300ms) — no request per keystroke. The page
  // resets here (with the search) so the query key changes atomically and we
  // never fetch a stale page-1 + page-N combo.
  useEffect(() => {
    const t = setTimeout(() => {
  ...
  const filters = useMemo(() => {
    ...
    if (segment === 'me' && teacherId) f.assignedTeacherId = teacherId;
    if (segment === 'unassigned') f.unassigned = true;
    if (search) f.search = search;
    return f;
  }, [segment, teacherId, status, subjectId, search]);

  // Reset to page 1 whenever filters change.
  const { data: doubtsData, isLoading } = useTeacherDoubts(filters, { page, pageSize: PAGE_SIZE });

  const doubts = doubtsData?.data ?? [];
  const totalCount = doubtsData?.count ?? 0;

  const { data: subjectOptions } = useTeacherDoubtSubjects();

  const goToDetail = useCallback(
    (doubt: StudentDoubt) => router.push(`/teacher/doubts/${doubt.doubtId}`),
    [router],
  );
  ...
  return (
    <div>
      <PageHeader
        title="Teacher Doubts"
        description="Review and respond to student questions from your assigned courses and batches."
      ...
      <div className="mb-4 flex flex-wrap items-end gap-3">
        ...
        <Select
          ...
          options={(subjectOptions ?? []).map((s) => ({ value: s.subjectId, label: s.name }))}
