-- ============================================================================
-- Migration: 177 — Fix Student Batch Subjects RPC Text Type Casting
-- ============================================================================
...
BEGIN
  ...
  student_entitled_courses AS (
    ...
    -- Permanent purchase entitlement
    SELECT
      ce.course_id
  ...
  SELECT
    ...
    -- Explicit text casts to match RETURNS TABLE
    COALESCE(s.name, 'General Subject')::text AS subject_name,
    COALESCE(b.name, 'Batch')::text AS batch_name,

    bcm.course_id,
    bcm.course_title::text AS course_title
...
COMMENT ON FUNCTION public.get_student_batch_subjects(uuid[]) IS
  'Returns authorized batch subjects with subject, batch, and course metadata for the authenticated student.';
