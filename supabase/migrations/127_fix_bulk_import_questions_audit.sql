-- ============================================================================
-- Migration 127: Fix Question Bank Bulk Import Audit Logging
--
-- Replaces public.bulk_import_questions_atomic RPC to use canonical
-- public.write_audit_log() instead of a direct insert with invalid column names
-- on public.audit_logs.
--
-- Preserves:
--   - Function signature, SECURITY DEFINER, search_path = public
--   - RBAC check: public.is_super_admin() OR public.is_academic_admin()
--   - Text-Based question support (correct_text_answer)
--   - MCQ, MSQ, True/False, and Numerical question support
--   - Atomic transaction and rollback guarantees
--   - Auto-publishing metadata (status='published', approved_by=p_actor)
-- ============================================================================
...
BEGIN
  ...
  -- 3. Log audit event for bulk import via canonical write_audit_log RPC
  PERFORM public.write_audit_log(
    p_action        => 'import'::public.audit_action_type,
    p_resource_type => 'questions',
    p_resource_id   => NULL,
