-- ============================================================================
-- Migration: 082 — Update get_home_screen_bootstrap RPC
--
-- Enhancements:
--   1. Expands `enrolled_courses` to unify directly enrolled courses
--      (public.course_enrollments) and batch-assigned courses (public.course_batches
--      via active public.batch_students).
--   2. Deduplicates courses when a course is both directly enrolled and batch-assigned,
--      prioritizing direct enrollment (priority 1 over priority 2).
--   3. Joins public.streams to include `category` (stream name) on each course.
--   4. Adds top-level `active_batches` array containing the student's active batch
--      memberships with `batch_id`, `name`, `batch_code`, and `stream_name`.
--   5. Preserves all existing keys (profile, unread_notifications_count, hero_banners,
--      trending_courses, featured_pyqs, demo_class) and their return structures.
-- ============================================================================
...
BEGIN
  ...
  -- Stream resolution: preference p_stream_id > profiles.selected_stream_id > student_details.selected_stream_id
  v_stream_id := p_stream_id;
  IF v_stream_id IS NULL THEN
    SELECT p.selected_stream_id INTO v_stream_id
    FROM public.profiles p
    WHERE p.profile_id = v_user_id;
  END IF;
  IF v_stream_id IS NULL AND v_student_id IS NOT NULL THEN
    SELECT sd.selected_stream_id INTO v_stream_id
  ...
  IF v_student_id IS NOT NULL THEN
    SELECT COALESCE(
      jsonb_agg(
        ...
        ORDER BY bs.enrolled_on DESC NULLS LAST
      ), '[]'::JSONB
    ) INTO v_active_batches
    FROM public.batch_students bs
    JOIN public.batches b ON bs.batch_id = b.batch_id
    LEFT JOIN public.streams s ON b.stream_id = s.stream_id
    WHERE bs.student_id = v_student_id
      AND bs.status = 'active'
      AND b.deleted_at IS NULL;
  END IF;

  -- 5. Enrolled courses (Unified: Direct enrollments + Batch-assigned courses)
  IF v_student_id IS NOT NULL THEN
    SELECT COALESCE(
      jsonb_agg(
        jsonb_build_object(
          'enrollment_id', uc.enrollment_id,
          'course_id', uc.course_id,
          'title', uc.title,
          'short_description', uc.short_description,
          'thumbnail_bucket', uc.thumbnail_bucket,
          'thumbnail_path', uc.thumbnail_path,
          'category', uc.stream_name,
          'status', uc.status,
          'enrolled_at', uc.enrolled_at,
          'source', uc.source
        )
        ORDER BY uc.enrolled_at DESC NULLS LAST
      ), '[]'::JSONB
    ) INTO v_enrolled_courses
    FROM (
      SELECT DISTINCT ON (c.course_id)
        combined.enrollment_id,
        c.course_id,
        c.title,
        c.short_description,
        c.thumbnail_bucket,
        c.thumbnail_path,
        COALESCE(s.name, 'Course') AS stream_name,
        c.status,
        combined.enrolled_at,
        combined.source
      FROM (
        -- Source 1: Direct course enrollments (Priority 1)
        SELECT 
          ce.enrollment_id,
          ce.course_id, 
          ce.enrolled_at, 
        ...
        -- Source 2: Batch-assigned courses (Priority 2)
        SELECT 
          NULL::UUID AS enrollment_id,
          cb.course_id, 
          cb.assigned_at AS enrolled_at, 
          'batch'::TEXT AS source, 
          2 AS priority
        FROM public.batch_students bs
        JOIN public.course_batches cb ON bs.batch_id = cb.batch_id
        WHERE bs.student_id = v_student_id 
          AND bs.status = 'active'
      ) combined
      JOIN public.courses c ON combined.course_id = c.course_id
      LEFT JOIN public.streams s ON c.stream_id = s.stream_id
      WHERE c.status = 'published' 
        AND c.deleted_at IS NULL
      ORDER BY c.course_id, combined.priority ASC, combined.enrolled_at DESC NULLS LAST
    ) uc;
  END IF;

  -- 6. Trending courses (scoped to stream)
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
  ...
  -- 7. Featured PYQ Packages (scoped to stream)
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
  ...
  -- 8. Demo Class (only for users who have NOT purchased/enrolled in any course)
  IF jsonb_array_length(v_enrolled_courses) > 0 THEN
  ...
  ELSIF v_user_id IS NOT NULL THEN
    ...
    IF NOT v_has_purchased AND v_student_id IS NOT NULL THEN
      SELECT EXISTS (
    ...
    IF NOT v_has_purchased AND v_student_id IS NOT NULL THEN
      SELECT EXISTS (
        ...
        WHERE student_id = v_student_id AND status IN ('active', 'grace')
        LIMIT 1
      ) INTO v_has_purchased;
    END IF;
  ELSE
    v_has_purchased := false;
  END IF;
